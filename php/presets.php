...
if ($_GET["action"] === "get_presets") {
    $q = $db->prepare('SELECT swmt, anons, registered, new, users, onlynew, regdays, oresFilter, editscount, namespaces, wlusers, wlprojects, blprojects, preset AS title FROM presets WHERE name = :userName GROUP BY preset;');
...
if ($_GET["action"] === "create_preset" && check($_GET["preset_name"])) {
    ...
    $q = $db->prepare('INSERT INTO presets (swmt, anons, registered, new, users, onlynew, regdays, oresFilter, editscount, namespaces, wlusers, wlprojects, blprojects, preset, name) VALUES (COALESCE(:swmt, 1), COALESCE(:anons, 1), COALESCE(:registered, 1), COALESCE(:new, 1), COALESCE(:users, 0), COALESCE(:onlynew, 0), COALESCE(:regdays, 5), COALESCE(:oresFilter, 0), COALESCE(:editscount, 100), COALESCE(:namespaces, null), COALESCE(:wlusers, null), COALESCE(:wlprojects, null), COALESCE(:blprojects, null), :preset, :name)');
    $q->execute($values);
...
if ($_GET["action"] === "edit_preset" && check($_GET["preset_name"])) {
    ...
    if ($q->rowCount() > 0) {
        ...
        $q = $db->prepare('UPDATE presets SET preset = :presetnew, swmt = COALESCE(:swmt, swmt), anons = COALESCE(:anons, anons), registered = COALESCE(:registered, registered), new = COALESCE(:new, new), users = COALESCE(:users, users), onlynew = COALESCE(:onlynew, onlynew), regdays = COALESCE(:regdays, regdays), oresFilter = COALESCE(:oresFilter, oresFilter), editscount = COALESCE(:editscount, editscount), namespaces = COALESCE(:namespaces, namespaces), wlusers = COALESCE(:wlusers, wlusers), wlprojects = COALESCE(:wlprojects, wlprojects), blprojects = COALESCE(:blprojects, blprojects), name = COALESCE(:name, name) WHERE preset = :preset AND name = :name');
        $q->execute($values);
        $q = $db->prepare('SELECT preset FROM presets WHERE name = :userName');
        $q->execute(array(':userName' => $userName));
        echo json_encode($q->fetchAll(PDO::FETCH_COLUMN));
...
if ($_GET["action"] === "get" && check($_GET["preset_name"])) {
    ...
    if ($q->rowCount() > 0) {
        $result = $q->fetchAll();
        echo json_encode(["blprojects" => $result[0]['blprojects'], "swmt" => $result[0]['swmt'], "onlyanons" => $result[0]['anons'], "users" => $result[0]['users'], "wlusers" => $result[0]['wlusers'], "wlprojects" => $result[0]['wlprojects'], "namespaces" => $result[0]['namespaces'], "registered" => $result[0]['registered'], "new" => $result[0]['new'], "onlynew" => $result[0]['onlynew'], "editcount" => $result[0]['editscount'], "regdays" => $result[0]['regdays'], " oresFilter" => $result[0][' oresFilter']]);
    } else
        echo json_encode(["result" => "error", "info" => "Not found"]);
}
